...
import { useHostServiceInspect } from '../api/hooks';
...
export default function HostServiceInspectTab({ project, service }: Props) {
  const { t } = useTranslation();
  const { data, isLoading, error } = useHostServiceInspect(project, service);
    ...
    <div className="flex flex-col gap-0">
      ...
      {config != null && (
        <Section title={t('inspect.config')}>
          <KeyValue label={t('inspect.image')} value={asString(safeGet(config, 'Image'))} />
