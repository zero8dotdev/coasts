...
export default function HostServiceStatsTab({ project, service }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(`/api/v1/host-service/stats/history?project=${encodeURIComponent(project)}&service=${encodeURIComponent(service)}`);
  ...
  useEffect(() => {
    ...
    const url = `${protocol}//${window.location.host}/api/v1/host-service/stats/stream?project=${encodeURIComponent(project)}&service=${encodeURIComponent(service)}`;
