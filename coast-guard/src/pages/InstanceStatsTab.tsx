...
export default function InstanceStatsTab({ project, name }: Props) {
  ...
  // WebSocket stream — replays history on connect, then streams live updates.
  // No separate REST fetch needed; the server sends the full history buffer
  // as the first messages over the WebSocket.
  useEffect(() => {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const url = `${protocol}//${window.location.host}/api/v1/stats/stream?project=${encodeURIComponent(project)}&name=${encodeURIComponent(name)}`;
